...
while (<>) {
	...
	if ($current_vers =~ /^REL[0-9_]*$/) {
	    ...
	    $vers_for_date = `cd /home/stark/src/postgres ; git describe --tags --abbrev=0 --match '${major}_0' $current_vers 2>/dev/null`;
	    $vers_for_date = `cd /home/stark/src/postgres ; git describe --tags --abbrev=0 --match '${major}' $current_vers 2>/dev/null`
		unless $vers_for_date;
	} else {
	    $vers_for_date = $current_vers;
	}

	my $gitinfo = `cd /home/stark/src/postgres ; git log -n 1 $vers_for_date`;
